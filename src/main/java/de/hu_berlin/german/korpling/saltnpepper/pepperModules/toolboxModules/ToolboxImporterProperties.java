 ...
 * spans for each annotation (key: 'newSpan', default value: 'false'), setting
 * the root element (key: 'rootElement', default value: 'database') and finally,
 * if one segment (e.g. one refGroup) holds more than one entry of primary text
 * and there are annotations, that shall be associated not only to the last
 * entry, you can define annotation layer (as an enumeration string), that shall
 * be associated to the primary text of the whole segment (key:
 * 'associateWithAllToks', default value: 'null')
 */

public class ToolboxImporterProperties extends PepperModuleProperties {
	// TODO remove serial id and ignore warning in eclipse, Java can doo that
	// automatrically, what is more error prune
	...
	public ToolboxImporterProperties() {
		addProperty(new PepperModuleProperty<>(PROP_TEXT_ELEMENT, String.class, "Defines the name of the xml tag that includes the textual data to be used as primary text (key: 'textElement', default value: 'unicode').", ToolboxXmlDictionary.TAG_UNICODE, false));
		addProperty(new PepperModuleProperty<>(PROP_SOUND_ELEMENT, String.class, "Defines the name of the sound recordings (key: 'audioRefElement', default value: 'sound').", ToolboxXmlDictionary.TAG_SOUND, false));
		addProperty(new PepperModuleProperty<>(PROP_SEGMENTING_ELEMENT, String.class, "Defines the element for segmenting into textualDS (key: 'segmentingElement', default value: 'refGroup').", ToolboxXmlDictionary.TAG_REF_GROUP, false));
		...
		addProperty(new PepperModuleProperty<>(PROP_TOKENIZE_TEXT, Boolean.class, "Defines, if the textual data shall be tokenized (key: 'tokenizeText', default value: 'true')", true, false));
		...
		addProperty(new PepperModuleProperty<>(PROP_NEW_SPAN, Boolean.class, "Defines, if there shall be created a new span for each annotation (key: 'newSpan', default value: false).", false, false));
		addProperty(new PepperModuleProperty<>(PROP_ROOT_ELEMENT, String.class, "Defines the name of root xml element (key: 'rootElement', default value: 'database').", ToolboxXmlDictionary.TAG_DATABASE, false));
	}
	...
	/**
	 * associate the enumerated, comma seperated annotations with the whole
	 * primary text of a segment, not only with the current one (only necessary
	 * for segments with more than one entry of primary text)
	 * 
	 * @return {@link String}
	 */
